...
// ── Экран "Поверни телефон" (только мобайл)
(function() {
  const rotateScreen = document.getElementById('rotateScreen');
  if (!rotateScreen) return;

  // Определяем мобайл по наличию тач и ширине
  ...
  // Скрыть экран при двойном тапе (если пользователь повернул)
  rotateScreen.addEventListener('touchend', () => {
    // Скроем только если уже landscape
  ...
  window.addEventListener('orientationchange', () => {
    setTimeout(checkOrientation, 300); // ждём пересчёт размеров
  });
