...
window.addEventListener('scroll', () => {
  ...
  if (cur > lastScrollY + 5)  setHeaderHidden(true);
  if (cur < lastScrollY - 5)  setHeaderHidden(false);
  lastScrollY = cur;
});
...
document.addEventListener('touchmove', (e) => {
  ...
  if (d > 10)  setHeaderHidden(true);
  if (d < -10) setHeaderHidden(false);
}, { passive: true });
...
btn2.addEventListener('click', () => {
  // Вычисляем счёт на основе количества правильных взмахов:
  // 0 взмахов = 0 → <50 (крыса + провал)
  // 1 взмах   = 33 → 50–80 (провал без крысы)  -- нет, 33 < 50 → крыса тоже
  // 2 взмаха  = 66 → 50–80 (провал без крысы)
  // 3+ взмахов (gestureComplete) = 100 → успех
  let score;
  if (gestureComplete) {
    score = 100;
  } else {
    // 0 → 0, 1 → 33, 2 → 66
    score = gestureStrokes.length * 33;
  }
  showLoading(score);
});
...
function showLoading(score) {
  ...
  setTimeout(() => {
    ...
    doFlash(() => showResult(score));
  }, 3000);
}

/* ---------- РЕЗУЛЬТАТ ---------- */
// score: число 0–100
// <50  → крыса остаётся видна + неудача.png + audioFail + modalFail
// 50–80 → крыса скрывается + неудача.png + audioFail + modalFail
// >80  → крыса скрывается + удача.png + audioSuccess + modalSuccess
function showResult(score) {
  const isSuccess    = score > 80;
  const showRat      = score < 50;   // крыса видна только при полном провале

  resultOverlay.src = isSuccess ? 'удача.png' : 'неудача.png';
  ...
  // Крыса скрывается, если score >= 50
  ratImg.style.opacity = showRat ? '1' : '0';
  ...
  audioBg.volume = 0.2;
  if (isSuccess) audioSuccess.play();
  else           audioFail.play();

  setTimeout(() => {
    ratImg.style.transition      = '';
    resultOverlay.style.transition = '';
    openModal(isSuccess ? 'modalSuccess' : 'modalFail');
  }, 2000);
}
